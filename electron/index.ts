 ...
 * ```
 */
export const setUpAppEnv = (options: SetupAppEnvParams) => {
  env = options;
};
/**
...
export const actions: Record<string, (e: Electron.IpcMainInvokeEvent) => void> =
  {
    ...
    toggleFullscreen: () =>
      getFocused()?.setFullScreen(!getFocused()?.isFullScreen()),
    fullscreen: () => getFocused()?.setFullScreen(true),
    unfullscreen: () => getFocused()?.setFullScreen(false),
  };
export const isDev = process.env.NODE_ENV === "development";
export const load = (win: BrowserWindow, ...urls: string[]) => {
  if (isDev && env.devUrl) {
...
export const windowInheritanceDefinition = async (
  topWebContents?: BrowserWindow["webContents"],
  index = 0,
) => {
  if (topWebContents) {
 ...
 * ```
 */
export const getBiqpodAppDataConfig = async (windowId: number) => {
...
export const getAllBiqpodAppDataConfig = async () => {
  ...
  try {
    ...
    const result: BiqpodAppDataConfig[] = JSON.parse(data);
    return result;
  } catch {
    return null;
  }
};
export const updateBiqpodAppDataConfig = async (
  windowId: number,
  data: BiqpodAppDataConfig,
) => {
...
export const createMainWindow = async (
  ...
  if (dataConfig?.bounds) win.setBounds(dataConfig.bounds);
  if (dataConfig?.isFullScreen) win.setFullScreen(true);
  if (dataConfig?.maximized) win.maximize();
  load(win);
  win.on("close", async () => {
...
export const startApplicationForDesktop = (
  callback: () => void | Promise<void>,
  options: BiqpodDesktopConfig = {},
) => {
  ...
  if (gotTheLock && options.appProtocol) {
    app.on("second-instance", (_e, argv) => {
      ...
      if (mainWindow) {
        const url = argv.find((url) =>
          url.startsWith(options.appProtocol!.concat("://")),
        );
        if (url) {
          mainWindow.maximize();
          mainWindow.focus();
  ...
  ipcMain.handle("action", (e, action: string) => actions[action]?.(e));
  ...
  ipcMain.handle("menu:show", async (_e, options) => {
    ...
    Menu.buildFromTemplate(
      options.menu?.map(({ index = 0, ...props }) => {
        return {
          click: () => {
